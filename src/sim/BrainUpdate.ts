import { GoalBattle, GoalFlee, GoalNoiseReact, GoalWander, GoalIdle, GoalTattle } from './goals/GoalTypes';
import { NoiseEvent } from './types';
...
export class BrainUpdate {
  ...
  private evaluateBrain(world: any): void {
    ...
    let mostThreateningAgent: any = null;
    ...
    if (mostThreateningAgent) {
      // Threat detected!
      const isCoward = this.agent.hasTrait('Coward') || this.agent.hasTrait('Pacifist');
      const isLowHealth = (this.agent.health / this.agent.maxHealth) < 0.25;

      if (isCoward || (isLowHealth && !this.agent.hasTrait('Aggressive'))) {
        if (!isFleeing) {
          this.agent.brain.pushGoal(new GoalFlee(this.agent, mostThreateningAgent, 6.0));
        }
      } else {
        if (!isBattling || (currentGoal as GoalBattle).target.id !== mostThreateningAgent.id) {
          this.agent.brain.pushGoal(new GoalBattle(this.agent, mostThreateningAgent));
        }
      }
      return;
    }
    ...
    const recentNoise: NoiseEvent | null = world.getRecentNoiseNear(this.agent.x, this.agent.y, this.agent.getHearingRadius());
    if (recentNoise) {
      const alreadyReacted = this.agent.brain.getMemory(`noise_${recentNoise.id}`);
